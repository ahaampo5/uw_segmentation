...
def build_model(CFG):
    if CFG.model_name == 'UNeXt':
        if CFG.kimin_model == True:
            ...
            model.to(CFG.device)
            return model
    elif CFG.model_name == 'UCTransNet':
        model = UCTransNet(n_channels=3, n_classes=CFG['num_classes'],img_size=512,vis=False).to(CFG.device)
        # checkpoint = torch.load('/VOLUME/jckim/uw_segmentation/pths/uc_pretrain/UCTransNet-MoNuSeg.pth')
        # print(checkpoint)
